...
export const LOVABLE_CANVAS_MOCKUP_PREVIEW_RUNTIME_VERSION = "2026-07-29.1";
...
function isDraftCandidateFile(projectRelativePath: string): boolean {
  if (!projectRelativePath.startsWith("src/") || !projectRelativePath.endsWith(".tsx")) return false;
...
// Keyed by filename so the canvas builds the same key from the file path alone.
function draftCandidateName(projectRelativePath: string): string {
  return (projectRelativePath.split("/").pop() ?? "").replace(/\.tsx$/, "");
}

// "button-candidate-squared-outline" -> "button": the component a draft explores.
function draftCandidateBaseName(name: string): string {
  const index = name.indexOf(DRAFT_CANDIDATE_INFIX);
...
// A draft's entry name is its filename; sample content should read as the component it
// explores, not as the file.
function previewDisplayName(name: string): string {
  const infixAt = name.indexOf(DRAFT_CANDIDATE_INFIX);
  return infixAt > 0 ? pascalCaseFromFileName(name.slice(0, infixAt)) : name;
}
...
function PreviewDocument({ previewPath }: { previewPath: string }): ReactElement | null {
  ...
  useEffect(() => {
    ...
    if (!entry) {
      setContent(errorContent('Component "' + previewPath + '" not found.'));
      return;
    }
    if (!entry.load) {
      setContent(noticeContent('"' + entry.name + '" has no source to preview (npm / Path B component).'));
      return;
    }
    entry
      .load()
      .then((mod) => {
        ...
        if (!Component) {
          setContent(errorContent('No component exported for "' + entry.name + '".'));
          return;
        }
...
> {
  ...
  render(): ReactElement {
    if (this.state.attempt === 0) return this.props.mount(true);
    if (this.state.attempt === 1) return this.props.mount(false);
    return errorContent("Component threw while rendering.");
  }
}

function displayNameOf(name: string): string {
  const infixAt = name.indexOf("-candidate-");
  if (infixAt <= 0) return name;
  return name
    .slice(0, infixAt)
    .split(/[-_.]+/)
    .filter(Boolean)
...
export function mockupPreviewPlugin(): Plugin {
  ...
  // Every .tsx under src/, minus the mockups tree (walk() owns that) and any
  // "_"-prefixed name the preview runtime refuses to serve.
...
<body>
  ...
  <script type="module">
    ...
    class RetryWithoutChildren extends React.Component {
      ...
      render() {
        if (this.state.attempt === 0) return mount(this.props.comp, true);
        if (this.state.attempt === 1) return mount(this.props.comp, false);
    ...
    import(${JSON.stringify(importPath).replace(/</g, "\\u003c")}).then((mod) => {
      ...
      if (Component) {
        ...
        root.render(React.createElement("pre", { style: { color: "red", padding: "2rem" } },
          "No exported component found for " + ${nameLiteral}));
      }
    }).catch((err) => {
      root.render(React.createElement("pre", { style: { color: "red", padding: "2rem" } },
        "Failed to load: " + err.message));
  ...
  // nor the barrel fallback can name them; glob them in separately, keyed by filename.
  ...
  function inheritExploredProps(draft: SchemaComponent, schema: SchemaComponent[]): SchemaComponent {
    const base = normalizeJoinKey(draftCandidateBaseName(draft.name));
    const explored = schema.find(
      (component) =>
        normalizeJoinKey(component.name) === base ||
        (component.file ? normalizeJoinKey(fileBaseName(component.file)) === base : false),
    );
